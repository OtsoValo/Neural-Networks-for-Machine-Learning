...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  %%%%%%%%%%%%%%%%
  sum_input_to_hid = model.input_to_hid * 0;
  sum_hid_to_class = model.hid_to_class * 0;
  wd_input_to_hid = model.input_to_hid * 0;
  wd_hid_to_class = model.hid_to_class * 0; 
  m = size(data.inputs, 2);
  for i=1:m
    hid_input = model.input_to_hid * data.inputs(:,i); % input to the hidden units, i.e. before the logistic. size: <number of hidden units> by <number of data cases>
    hid_output = logistic(hid_input); % output of the hidden units, i.e. after the logistic. size: <number of hidden units> by <number of data cases>
    class_input = model.hid_to_class * hid_output; % input to the components of the softmax. size: <number of classes, i.e. 10> by <number of data cases>
  ...
  % probability of each class. Each column (i.e. each case) sums to 1. size: <number of classes, i.e. 10> by <number of data cases>
    ...
    delta_class = class_prob - data.targets(:,i);
    delta_hid = ((model.hid_to_class)' * delta_class) .* (logistic(hid_input).*(1-logistic(hid_input)));
    sum_hid_to_class = delta_class * hid_output' + sum_hid_to_class;
    sum_input_to_hid = delta_hid * (data.inputs(:,i))' + sum_input_to_hid; 
  end
    wd_hid_to_class = model.hid_to_class .* wd_coefficient;
